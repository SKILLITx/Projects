...
function reasonSentence(dealer, reason) {
  ...
  switch (reason.factor) {
    case "Late and inconsistent payment timing":
      detail = `Averages ${Number(dealer.avg_days_late_nonseasonal).toFixed(1)} days late on payments (excluding Eid/Ramzan), with payment timing varying by roughly ${Number(dealer.payment_volatility).toFixed(1)} days — making cash flow hard to predict.`;
      break;
    ...
    case "Elevated risk in dealer's territory":
      ...
      break;
    default:
      // Covers the cold-start explanation text, which is already a full
      // sentence rather than a short factor label -- show it plainly
      // without the increases/reduces-risk framing used for statistical reasons.
      return { header: reason.factor, detail: "" };
  }
