...
for dealer_id in insufficient_feature_history:
    ...
    cold_start_rows.append({
        ...
        "salesman_id": d["salesman_id"], "is_salesman_favorite": d["is_salesman_favorite"],
...
cold_start_df = pd.DataFrame(cold_start_rows, columns=[
    ...
    "credit_score", "risk_flag", "scoring_method",
])

# ---------------------------------------------------------------------------
# STEP 6: UNIFY into a single output covering ALL 220 dealers
# ---------------------------------------------------------------------------
unified_cols = ["dealer_id", "dealer_name", "city", "salesman_id",
                 "is_salesman_favorite", "credit_score", "risk_flag", "scoring_method"]

statistical_out = feat_df[unified_cols]
unified = pd.concat([statistical_out, cold_start_df[unified_cols]], ignore_index=True)
unified = unified.sort_values("credit_score").reset_index(drop=True)
